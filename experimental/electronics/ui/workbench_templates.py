"""
workbench_templates.py — Modular HTML, CSS, and JS template components for Silicon Workbench.
"""
...
def render_workbench_styles() -> str:
    """Renders standalone CSS style rules."""
    return """  <style>
    :root {{
      --bg: #070d19;
      --card: #0f172a;
...
def render_workbench_header(ctx: dict[str, Any]) -> str:
    """Renders top status bar, mode switcher pill, and telemetry gauges."""
    title = ctx["title"]
    scenario_name = ctx["scenario_name"]
    tech_node = ctx["tech_node"]
    fitness = ctx["fitness"]
    generations = ctx["generations"]
    nom_metrics = ctx["nom_metrics"]

    return f"""  <header class="bg-[#0f172a] border border-[#1e293b] rounded-xl p-4 shadow-xl flex flex-wrap items-center justify-between gap-4">
...
def render_analog_studio(ctx: dict[str, Any]) -> str:
    """Renders Analog Studio view."""
    nom_metrics = ctx["nom_metrics"]
    ss_metrics = ctx["ss_metrics"]
    ff_metrics = ctx["ff_metrics"]
    analog_sizing = ctx["analog_sizing"]
    spice_code = ctx["spice_code"]

    return f"""  <div id="view-analog" class="flex flex-col gap-6 flex-1">
...
def render_digital_studio(ctx: dict[str, Any]) -> str:
    """Renders Digital Studio view."""
    verilog_code = ctx["verilog_code"]
    spice_code = ctx["spice_code"]
    fpga_board_name = ctx["fpga_board_name"]
    fpga_vendor = ctx["fpga_vendor"]
    fpga_lut_ratio = ctx["fpga_lut_ratio"]
    fpga_fmax = ctx["fpga_fmax"]
    fpga_delay = ctx["fpga_delay"]
    fpga_pins = ctx["fpga_pins"]
    fpga_power = ctx["fpga_power"]
    fpga_fit = ctx["fpga_fit"]

    return f"""  <div id="view-digital" class="hidden flex flex-col gap-6 flex-1">
...
def render_client_scripts(ctx: dict[str, Any]) -> str:
    """Renders client-side dual-mode JavaScript engine and virtual instruments."""
    cgp_json = ctx["cgp_json"]
    netlist_json = ctx["netlist_json"]
    analog_json = ctx["analog_json"]
