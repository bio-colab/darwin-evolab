...
def _build_boolean_expr_scenario(
    boolean_src: str,
    obj_matrix: Any,
    population_size: int,
    rng: random.Random,
    ...
    return evaluator, pop, "custom_boolean_expr"


def _build_verilog_scenario(
    v_src: str | Path,
    obj_matrix: Any,
    population_size: int,
    rng: random.Random,
    ...
    return evaluator, pop, f"verilog_{v_spec.module_name}"


def _build_waveform_scenario(
    wf_src: str | Path,
    population_size: int,
    rng: random.Random,
) -> tuple[Any, list[Individual], str]:
...
def _build_analog_filter_scenario(
...
def _build_legacy_digital_scenario(
    spec_data: dict[str, Any],
    spec_file: Path | None,
    population_size: int,
    rng: random.Random,
...
def _build_legacy_analog_scenario(
    spec_data: dict[str, Any],
    spec_file: Path,
    population_size: int,
    rng: random.Random,
...
def prepare_custom_electronics_run(
    ...
    # Mode 1: Boolean Logic Expressions
    boolean_src = expr or spec_data.get("expressions") or spec_data.get("expr")
    if boolean_src:
        return _build_boolean_expr_scenario(boolean_src, obj_matrix, population_size, rng)

    # Mode 2: Synthesizable Verilog RTL
    v_src = verilog_in or spec_data.get("verilog_in") or spec_data.get("verilog_code")
    if v_src:
        return _build_verilog_scenario(v_src, obj_matrix, population_size, rng)

    # Mode 3: Target Oscilloscope Waveform CSV
    wf_src = waveform_path or spec_data.get("waveform_csv")
    if wf_src:
        return _build_waveform_scenario(wf_src, population_size, rng)

    # Mode 4: Analog Filter Specification
    if spec_data and any(k in spec_data for k in ("cutoff_hz", "filter_type")):
        return _build_analog_filter_scenario(spec_data, population_size, rng)

    # Mode 5: Legacy Digital truth_table
    if "truth_table" in spec_data:
        return _build_legacy_digital_scenario(spec_data, spec_file, population_size, rng)

    # Mode 6: Legacy Analog design_vars or specs
    if spec_file and ("design_vars" in spec_data or "specs" in spec_data):
        return _build_legacy_analog_scenario(spec_data, spec_file, population_size, rng)
