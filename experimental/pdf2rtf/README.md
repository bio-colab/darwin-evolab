...
6. **حل دقة تباعد الأسطر التيبوغرافي في وورد (`Layout Line Spacing Resolution`)**:
   ...
   - **الحل النهائي المطبق**: تم تحديث `word_holdout.py` ليستخرج التباعد التيبوغرافي الفعلي مباشرة من محرك التخطيط المادي لـ Word عبر `p.Range.ComputeStatistics(wdStatisticLines)` وإحداثيات الأسطر الرأسية `p.Range.Characters(c).Information(wdVerticalPositionRelativeToPage)`:
     - الفقرات ذات السطر الواحد (`num_lines <= 1`): يُسجّل تباعد الأسطر الداخلي كـ `None` (تطابقاً تاماً مع منطق `PDFExtractor`).
     - الفقرات متعددة الأسطر (`num_lines > 1`): تُحسب المسافة الرأسية الحقيقية بين خطوط الأساس ($\Delta y$): تُسجّل بدقة `15.5 pt` لخط Calibri 11pt، و `16.0 pt` لخط Times New Roman 12pt.
   - **التطابق التجريبي**: تطابقت قيم الحقيقة المرجعية المستخرجة من تخطيط Word بنسبة 100% وبدقة الكسور العشرية مع ما يستخرجه `PDFExtractor` عبر PyMuPDF، مما جعل الحقيقة المرجعية موثوقة تماماً لأي معايرة مستقبلية.
...
|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **word_academic_paper** | **100.00%** | 97.50% | 100.00% | 99.38% | 100.00% | **99.41%** | 19.2 ms | **PASSED** ✅ |
| **word_financial_report** | **100.00%** | 95.62% | 100.00% | 100.00% | 100.00% | **99.12%** | 28.2 ms | **PASSED** ✅ |
| **word_executive_letter** | **100.00%** | 97.19% | 100.00% | 97.90% | 100.00% | **99.12%** | 16.8 ms | **PASSED** ✅ |
| **word_styled_article** | **100.00%** | 94.58% | 100.00% | 94.38% | 100.00% | **98.07%** | 13.4 ms | **PASSED** ✅ |
| **المتوسط الإجمالي (Holdout)** | **100.00%** | **96.22%** | **100.00%** | **97.92%** | **100.00%** | **98.93%** | **19.4 ms** | **100% ALL PASS** ✅ |
