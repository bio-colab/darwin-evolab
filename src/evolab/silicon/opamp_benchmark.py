...
def generate_opamp_spice_netlist(
    ...
    rz_kohm: float = 0.0,
    title_comment: str = "* Two-Stage Miller OpAmp for SkyWater 130nm\n* Generated by Darwin-Evolab Silicon Engine",
) -> str:
...
class Sky130OpAmpAdapter(DomainAdapter):
    ...
    def parse_spec(self, raw_spec: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "use_surrogate": bool(d.get("use_surrogate", False)),
        }
    ...
    def build_population(
        ...
        if use_spec_cond and has_targets:
            try:
                ...
                pop = prior.sample_seed_population(count=size, species="sky130_opamp")

                if spec.get("use_bandit", False):
                    from .bandit_latent_optimizer import BanditLatentOptimizer
                    bandit = BanditLatentOptimizer(seed=42)
                    sizings = [OpAmpSizing.from_normalized_vector(ind.genome.values) for ind in pop]
                    opt_sizings = bandit.optimize_seeds(sizings, target_spec=target_spec, steps=int(spec.get("bandit_steps", 15)))
                    pop = [
                        Individual(
                            genome=FloatGenome(values=s.to_normalized_vector()),
                            species="sky130_opamp",
                            fitness=0.0,
                            _generation=0,
                            _index=idx,
                        )
                        for idx, s in enumerate(opt_sizings)
                    ]
                return pop
            except Exception:
    ...
    def build_evaluator(self, spec: dict[str, Any]) -> Evaluator:
        ...
        if spec.get("use_surrogate", False):
            from .surrogate import ActiveSpiceSurrogateEvaluator
            evaluator = ActiveSpiceSurrogateEvaluator(base_evaluator=evaluator)
